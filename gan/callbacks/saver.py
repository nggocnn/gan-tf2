...
class ImageProblemSaver(Callback):

    def __init__(self, save_images_every_n_steps: int, num_test_examples: int = None):
        super().__init__()
    ...
    def on_training_step_end(self, trainer):
        if trainer.global_step % self.save_images_every_n_steps == 0:
            for name, generator in trainer.generators.items():
                ...
                img_to_plot = visualizer.generate_and_save_images_for_image_problems(
...
class FunctionProblemSaver(Callback):
    ...
    def on_training_step_end(self, trainer):
        if trainer.global_step % self.save_images_every_n_steps == 0:
            for name, generator in trainer.generators.items():
                ...
                img_to_plot = visualizer.generate_and_save_images_for_model_fn_problems(
                    generator_model=generator,
                    epoch=trainer.global_step,
                    test_input=trainer.validation_dataset,
